...
void handle_sigint(int sig) {
    for (int i = 0; i < NUM_DEPARTMENTS; ++i) {
        sem_destroy(departments[i]);
        sem_destroy(customer_arrived[i]);
    }
    exit(0);
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < NUM_DEPARTMENTS; ++i) {
        departments[i] = (sem_t*)mmap(NULL, sizeof(sem_t), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
        customer_arrived[i] = (sem_t*)mmap(NULL, sizeof(sem_t), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
        sem_init(departments[i], 1, 1);
        sem_init(customer_arrived[i], 1, 0);
        pid_t pid = fork();
        if (pid == 0) {
            seller(i);
            return 0;
