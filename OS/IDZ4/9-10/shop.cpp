...
void finish(int signum) {
    for (int i = 0; i < departments.size(); i++) {
        message mess = {3, 0, 0, ""};
        sendto(server_sock, &mess, sizeof(mess), 0, (struct sockaddr *) &departments[i], sizeof(departments[i]));
    }
    for (int i = 0; i < customers.size(); i++) {
        message mess = {3, 0, 0, ""};
        sendto(server_sock, &mess, sizeof(mess), 0, (struct sockaddr *) &customers[i], sizeof(customers[i]));
    }
    for (int i = 0; i < outputs.size(); i++) {
        message mess = {3, 0, 0, ""};
        sendto(server_sock, &mess, sizeof(mess), 0, (struct sockaddr *) &outputs[i], sizeof(outputs[i]));
    }
    close(server_sock);
    exit(0);
}
